...
class IntruderDetector(FileStorageUnit):
    def __init__(self, training_image_path, data_list_file_path, image_dim_width=128, image_dim_length=128, notifiers_list=[]):
        ...
        """
        self.__is_intruder_threshold_list = self.get_data_list() # pointer to the same list
    ...
    def train_model(self):
        ...
        # images grouped in batches for classification
        ...
        if self.num_classes == 0:
            return
        print(f'num_classes2: {self.num_classes}')
        ...
        #self.__model.add(Dense(self.__image_dim_width, activation='relu'))
    ...
    def classify(self, image_path=None, test_image=""):
        # user must specify either one of impage_path or test_image
        # returns prediction and probabilty of prediction
        if self.num_classes==0:
            return "no_classes", 1
        if image_path:
    ...
    def is_intruder(self, prediction, probabilty):
        # print("prob")
        print(probabilty)
        # print(self.__is_intruder_threshold_list[0])
    ...
    def get_notifier(self, intruder_info):
